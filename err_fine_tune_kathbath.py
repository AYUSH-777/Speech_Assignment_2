...
def validate(model, val_loader):
    ...
    with torch.no_grad():
        ...
        for data, target in val_loader:
            output = model(data)
            # Calculate EER using speaker verification
            similarity_score = speaker_verification(output)
            y_true.append(target.item())
            y_score.append(similarity_score)
            # Update total and correct samples count
            total_samples += target.size(0)
            correct_samples += (output.argmax(1) == target).sum().item()

    eer_value = calculate_eer(y_true, y_score)
    return eer_value
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        eer_value = validate(model, val_loader)
        print(f"Epoch [{epoch+1}/{num_epochs}], EER: {eer_value}%")

    print(f"Final EER on validation set: {eer_value}%")
